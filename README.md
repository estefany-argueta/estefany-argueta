# Hi, I’m Estefany Argueta Herrera

I’m a molecular ecologist studying **thermal adaptation in marine ectotherms under climate change**. I combine **experimental physiology, field ecology, and genetics** to build mechanistic predictions about which populations persist as oceans warm.

**Currently:** PhD candidate at the University of Massachusetts Amherst.

---

## What drives my work
Some of my best science happens outdoors—wet gear, salty notebooks, and early mornings in places that feel special. I’m motivated by questions that connect **fundamental physiology** to **real conservation outcomes**, especially as climate change pushes marine systems into new territory.

---

## Research projects
### 1) Mating Behaviors as Buffers for Climate Impacts in Sea Turtle Populations
**Question:** Can male behaviors compensate for potentially increasing male scarcity?
**Approach:** kinship/genetics, field monitoring, sexual selection 
**Why it matters:** sex ratio skew is one of the clearest climate signals and it can reshape populations quickly.

### 2) Cold tolerance in range-expanding black sea bass
**Question:** Are northern-edge populations genetically adapting to cold, or relying on plasticity?
**Approach:** experimental physiology, field collections, biological hierarchy  
**Why it matters:** range shifts are happening now—mechanisms determine who persists during extremes.

---

## Tools & workflows
- **R** for statistics, visualization, and reproducible analyses
- Version control (Git/GitHub) + documented, shareable pipelines
- Bioinformatics / genetics workflows (general genomics support as needed)

---

## Collaboration + values
I care about clear communication, reproducible science, and collaborative work that’s thoughtful and kind. I’m also committed to **promoting diversity and inclusion in science** because better science is built by broader participation and equitable opportunities.

---

## Find me / contact
- Email: earguetaherr@umass.edu
- Google Scholar: Estefany Argueta[https://scholar.google.com/citations?user=Asvvy_gAAAAJ&hl=en]
- ORCID: 0009-0002-0790-3256[https://orcid.org/0009-0002-0790-3256]
- CV: [link to PDF]
- Website: <https://github.com/estefany-argueta>

---

## A little more human
- I’m happiest doing fieldwork and then turning messy data into clean, honest figures.
- I love research that lets me return to (and learn from) remarkable places in nature.
